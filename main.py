...
if (input("pretrain? (y/n)") == "y"):
    ...
    model = MaskablePPO.load("ppo_models/" + model_name, env=train_env, verbose=1, batch_size=1024, n_steps=512, learning_rate=1e-4, policy_kwargs=policy_kwargs, tensorboard_log="./tensorboard/")
    print("model loaded")
else:
    ...
    train_env = ActionMasker(GomokuEnv(render=False), mask_fn)
    env_eval = ActionMasker(GomokuEnv(render=False), mask_fn) 
    model = MaskablePPO(CustomActorCriticPolicy, env=train_env, verbose=1, batch_size=1024, n_steps=512, earning_rate=1e-4, policy_kwargs=policy_kwargs, tensorboard_log="./tensorboard/")
    print("model created")
