...
if (input("pretrain? (y/n)") == "y"):
    ...
    model = MaskablePPO.load("ppo_models/" + model_name, env=train_env, verbose=1, n_steps=128, learning_rate=1e-4, policy_kwargs=policy_kwargs, tensorboard_log="./tensorboard/")
    print("model loaded")
else:
...
eval_callback = CustomMaskableEvalCallback(eval_env=env_eval, best_model_save_path="./best_ppo_models/", log_path="./callback_logs/", eval_freq=1024, deterministic=False, n_eval_episodes=2)
